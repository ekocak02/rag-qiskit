...
class NotebookProcessor:
    ...
    def _compile_patterns(self):
        ...
        self.tooltip_pattern = re.compile(
            r"<DefinitionTooltip.*?>(.*?)</DefinitionTooltip>", re.DOTALL
        )
        self.admonition_pattern = re.compile(
            r"<Admonition.*?>(.*?)</Admonition>", re.DOTALL
        )
        self.details_pattern = re.compile(
            r"<details>\s*<summary>(.*?)</summary>(.*?)</details>", re.DOTALL
        )
        self.import_pattern = re.compile(r"^\s*(?:import|from)\s+(\w+)", re.MULTILINE)
        self.noise_patterns = [
    ...
    def _extract_content(self, notebook_data: Dict, filename: str) -> Dict[str, Any]:
        ...
        for cell in notebook_data.get("cells", []):
            ...
            if cell.get("cell_type") == "markdown":
                ...
                if clean_text:
                    processed_cells.append({"type": "text", "content": clean_text})
    ...
    def _clean_markdown(self, text: str) -> str:
        ...
        text = self.admonition_pattern.sub(
            lambda m: f"\n> **Note:** {m.group(1).strip()}\n", text
        )
        text = self.details_pattern.sub(
            lambda m: f"\n**{m.group(1).strip()}**\n{m.group(2).strip()}\n", text
        )
    ...
    def _process_code_cell(self, cell: Dict, source_text: str) -> Dict[str, str]:
        ...
        return {"type": "code", "content": code_block, "output": clean_output}
    ...
    def _determine_qiskit_version(
        self, libraries: Set[str], raw_code: str
    ) -> Optional[str]:
