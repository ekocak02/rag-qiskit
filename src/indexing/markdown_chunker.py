...
class MarkdownProcessor(BaseProcessor):
    ...
    def _process_single_item(self, item: Dict[str, Any]) -> List[ProcessedChunk]:
        ...
        for doc in header_docs:
            ...
            if self.token_helper.count_tokens(doc.page_content) > self.token_helper.safe_limit:
            ...
            for sub_doc in sub_docs:
                ...
                smart_chunks = self.smart_unmask_and_split(sub_doc.page_content, base_meta)
                
                for chunk in smart_chunks:
                    chunk.metadata["has_code"] = "```" in chunk.page_content
                    chunk.metadata["has_latex"] = "$" in chunk.page_content or "LATEX" in chunk.page_content
                    final_chunks.append(chunk)

        return final_chunks

    def _build_metadata(self, header_meta: Dict, original_item: Dict, content: str) -> Dict:
    ...
    def _convert_html_headers(self, text: str) -> str:
        def replace(m): return '#' * int(m.group(1)) + ' ' + m.group(2)
