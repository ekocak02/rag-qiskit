...
def mock_marker_library():
    with patch("src.ingestion.pdf_processor.create_model_dict"), \
         patch("src.ingestion.pdf_processor.PdfConverter") as mock_converter_cls, \
         patch("src.ingestion.pdf_processor.text_from_rendered") as mock_text_extractor:
...
def clean_env():
    ...
    if "TORCH_DEVICE" in os.environ: del os.environ["TORCH_DEVICE"]
    yield
    if old_device: os.environ["TORCH_DEVICE"] = old_device

@pytest.fixture
def processor(mock_filesystem, clean_env):
    return PDFProcessor(
        raw_dir=str(mock_filesystem["raw"]),
        output_dir=str(mock_filesystem["output"])
    )
...
def test_process_file_integrates_cleaning(processor, mock_filesystem, mock_marker_library):
    """
