...
class MyClass:
    ...
    class_chunk = next(
        c for c in chunks if c.metadata.get("type") == "class_definition"
    )
    assert "class MyClass" in class_chunk.page_content
    assert "def __init__" in class_chunk.page_content

    method_chunk = next(
        c for c in chunks if c.metadata.get("function_name") == "method_one"
    )
...
def func(): pass
    ...
    module_chunk = next(
        (c for c in chunks if c.metadata.get("type") == "module_level"), None
    )
