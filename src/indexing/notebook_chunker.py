...
class NotebookProcessor(BaseProcessor):
    ...
    def process_file(self, file_content: str) -> List[ProcessedChunk]:
        ...
        for i, block in enumerate(blocks):
            ...
            if tokens > self.token_helper.safe_limit:
                # Flush buffer first
                if buffer_text:
                    chunks.extend(self._flush_buffer(buffer_text, file_meta, start_idx, i-1))
                    buffer_text, buffer_tokens = [], 0
                
                # Process this large cell individually
                chunks.extend(self._process_large_block(block_text, file_meta, i, block.get("type", "text")))
                start_idx = i + 1
                continue
        ...
        if buffer_text:
            chunks.extend(self._flush_buffer(buffer_text, file_meta, start_idx, len(blocks)-1))
        return chunks

    def _flush_buffer(self, text_list: List[str], meta: Dict, start: int, end: int) -> List[ProcessedChunk]:
    ...
    def _process_large_block(self, text: str, base_meta: Dict, idx: int, b_type: str) -> List[ProcessedChunk]:
        """
        ...
        if b_type == "text":
            # Mask Latex
            masked = self.mask_sensitive_blocks(text, [(r'\[LATEX_START\].*?\[LATEX_END\]', "LATEX")])
