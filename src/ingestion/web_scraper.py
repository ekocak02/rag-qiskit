...
class WebScraper:
    ...
    def _process_code_blocks(self, prose: Tag) -> None:
        ...
        code_blocks = prose.find_all('div', attrs={'data-rehype-pretty-code-fragment': True})
    ...
    def parse_content(self, html: str, url: str) -> Optional[Dict]:
        ...
        topic = h1_tag.get('id') if h1_tag and h1_tag.has_attr('id') else "unknown_topic"
        if topic == "unknown_topic" and h1_tag:
             topic = re.sub(r'[^a-z0-9]+', '-', h1_tag.get_text(strip=True).lower()).strip('-')

        has_code = bool(prose.find('div', attrs={'data-rehype-pretty-code-fragment': True}))
